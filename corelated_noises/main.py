...
# Uninstall threadpoolctl
# subprocess.run([sys.executable, "-m", "pip", "uninstall", "threadpoolctl", "-y"], check=True)

# Install a specific version
subprocess.run([sys.executable, "-m", "pip", "install", "threadpoolctl==3.1.0"], check=True)
...
# topologies = [("centralized", "ldp") , ("centralized", "corr"),  ("centralized", "fdp"),
#               ("grid", "ldp"), ("grid", "corr"), ("grid", "fdp"), 
#               ("ring", "ldp"), ("ring", "corr"), ("ring", "fdp")]
topologies = [("grid", "ldp"), ("grid", "corr"), ("grid", "fdp"), 
              ("ring", "ldp"), ("ring", "corr"), ("ring", "fdp")]
alphas = [10]
# epsilons = [0.1, 0.5, 1, 3]
# tick_labels =  [0.1, 0.5, 1, 3]


epsilons = [1,3, 5, 7, 10, 15, 20, 25, 30, 40]
tick_labels = [1,3, 5, 7, 10, 15, 20, 25, 30, 40]
...
#                     ("centralized", "fdp", 0.1) : (5, 1), ("centralized", "fdp", 0.5): (5, 1), ("centralized", "fdp", 1) : (5, 1), ("centralized", "fdp", 3): (5, 1),
#                    ("grid", "fdp", 0.1) : (5, 1), ("grid", "fdp", 0.5): (5, 1), ("grid", "fdp", 1) : (1, 5), ("grid", "fdp", 3): (1, 5), 
#                    ("ring", "fdp", 0.1) : (2, 1), ("ring", "fdp", 0.5): (1, 1.5), ("ring", "fdp", 1) : (1, 1.5), ("ring", "fdp", 3): (1, 1.5)
# }
...
from findSigma_original import compute_sigma_fdp
        ...
        for target_eps in epsilons:
            for topology_name, method in topologies:
                ...
                if "corr" in method: # CD-SGD
                    ...
                    print("=====>Their code\n", method, "sigma=", params["sigma"],"\nsigma_cor=", params["sigma-cor"])
                    
                    # Store result
                    filename= f"result_mnist_gridsearch_user-level_{topology_name}_epsilon_{target_eps}.csv"
                    df.to_csv(filename)
                ...
                elif "fdp" in method: # fDP
                    params["sigma"] = compute_sigma_fdp(params["num-iter"], adjacency_matrix, sigma_cdp, params["sigma-cor"], params["gradient-clip"], delta=params["delta"], bracket=(1e-7, 1000))
                    jobs.submit(f"{dataset}-{topology_name}-{method}-n_{params['num-nodes']}-model_{model}-alpha_{alpha}-eps_{target_eps}", make_command(params))
                
                    print(f"=====>Our code\n target_eps = {target_eps}, topo={topology_name}\n",
                            "sigma_fdp =", params["sigma"], "\n",)
...
method_to_color = {"ldp": "tab:orange", "cdp": "tab:purple", "corr": "tab:green", "fdp": "tab:red"}
method_to_marker = {"ldp": "^", "cdp": "D", "corr": "o", "fdp": "D"}
method_to_legend = {"ldp": "LDP", "corr": "DECOR", "cdp": "CDP", "fdp": "f-DP"}
